...
export function MobileShell({ tab, onTab, children }: { tab: MobileTab; onTab: (tab: MobileTab) => void; children: ReactNode }) {
  return (
    <main className="min-h-screen bg-[#02040a] text-white">
      <div className="mx-auto min-h-screen max-w-[430px] overflow-hidden bg-[radial-gradient(circle_at_22%_-6%,rgba(59,130,246,0.22),transparent_34%),radial-gradient(circle_at_90%_8%,rgba(96,165,250,0.10),transparent_28%),linear-gradient(160deg,#070b13_0%,#05070d_48%,#0b1220_100%)] shadow-[0_0_90px_rgba(0,0,0,0.88)]">
        <StatusBar />
        <div className="px-4 pb-32 text-[15px] leading-snug min-[400px]:px-5">{children}</div>
        <nav className="fixed inset-x-0 bottom-0 z-30 mx-auto max-w-[430px] border-t border-white/10 bg-[#0b111d]/85 px-3 pb-[calc(env(safe-area-inset-bottom)+10px)] pt-2 shadow-[0_-22px_70px_rgba(0,0,0,0.62)] backdrop-blur-2xl min-[400px]:px-4">
          <div className="grid grid-cols-5 gap-1 rounded-[1.75rem] border border-white/[0.06] bg-white/[0.035] p-1">
            {tabs.map((item) => {
              ...
              return (
                <button key={item.tab} onClick={() => onTab(item.tab)} className={`min-h-[4rem] rounded-2xl px-1 text-[0.68rem] font-semibold transition active:scale-[0.98] ${active ? "bg-white/[0.08] text-[#3b82f6] shadow-[inset_0_0_30px_rgba(59,130,246,0.18),0_10px_30px_rgba(0,0,0,0.24)]" : "text-slate-300 hover:text-white"}`}>
