...
export class AppErrorBoundary extends Component<{ children: ReactNode }, AppErrorBoundaryState> {
  ...
  static getDerivedStateFromError(error: unknown): AppErrorBoundaryState {
    return { message: error instanceof Error ? error.message : "IronLung Analyzer hit a browser runtime error." };
  }

  componentDidCatch(error: unknown, info: ErrorInfo) {
    console.error("IronLung Analyzer runtime error", error, info);
  }
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-ink p-6 text-white">
        <section className="w-full max-w-sm rounded-2xl border border-red-400/30 bg-red-500/10 p-5 text-center">
          <div className="text-lg font-black">IronLung Analyzer could not open</div>
