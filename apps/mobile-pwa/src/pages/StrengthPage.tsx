...
export function StrengthPage({ snapshot, analyzer }: AnalyzerPageProps) {
  return (
    <div className="space-y-4">
      ...
      <MobileCard>
        ...
        <StrengthRows rows={analyzer.strengthRows.slice(0, 8)} unit={snapshot.settings.unitPreference} />
      </MobileCard>
      <MobileCard>
        <div className="mb-3 font-bold">Top by max weight</div>
        <StrengthRows rows={analyzer.maxWeightRows.slice(0, 6)} unit={snapshot.settings.unitPreference} metric="max" />
      </MobileCard>
      <MobileCard>
        <div className="mb-3 font-bold">Major / medium PRs</div>
        <PrList snapshot={snapshot} prs={analyzer.recentPrs.filter((record) => record.importance === "major" || record.importance === "medium").slice(0, 6)} />
      </MobileCard>
      <MobileCard>
