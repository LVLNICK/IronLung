    ...
    <div className="space-y-4">
      <header className="flex items-start justify-between pt-1">
        ...
        <div className="text-center">
          <div className="text-2xl font-black">Upper Strength — Active</div>
      ...
      <section className="rounded-[1.55rem] border border-white/12 bg-[linear-gradient(135deg,rgba(26,35,51,.92),rgba(10,15,24,.94))] p-4">
        <div className="flex items-start justify-between">
          <div className="flex gap-4">
            ...
            <div>
              <div className="text-2xl font-black">Bench Press</div>
              <div className="mt-1 text-slate-400">Chest • Triceps • Front Delts</div>
            </div>
          </div>
          <div className="flex items-center gap-3"><span className="rounded-full bg-blue-500 px-4 py-2 font-bold">4 of 4 sets</span><ChevronDown /></div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-[1fr_1.4fr_1fr_1fr_1.3fr_2rem] px-4 text-[0.65rem] font-black uppercase tracking-widest text-slate-500">
          <span>Set</span><span>Weight</span><span>Reps</span><span>RPE</span><span>e1RM</span><span />
        </div>
        <div className="mt-2 space-y-1.5">
          {rows.map((row) => (
            <div key={row[0]} className={`grid grid-cols-[1fr_1.4fr_1fr_1fr_1.3fr_2rem] items-center rounded-xl border border-white/10 px-4 py-3 font-mono text-lg ${row[0] === "4" ? "border-l-4 border-l-blue-500 bg-blue-500/10" : "bg-black/12"}`}>
        ...
        <div className="mt-5 rounded-2xl border border-blue-500/45 bg-black/20 p-4">
          ...
          <div className="mt-3 grid grid-cols-[1fr_1fr_1.8fr_1.8fr] gap-2">
      ...
      {[
        ...
        <div key={item[0]} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[0.045] p-3">
          <ExerciseIcon icon={Dumbbell} />
          <div className="flex-1"><div className="text-xl font-black">{item[0]}</div><div className="text-sm text-slate-400">{item[1]}</div></div>
          <div className={`font-bold ${item[3]}`}>{item[2]}</div><ChevronDown />
        </div>
      ))}
      <button className="flex h-16 w-full items-center justify-center gap-3 rounded-2xl bg-blue-500 text-xl font-black shadow-[0_0_32px_rgba(59,130,246,0.4)]"><Check className="rounded-full border border-white" />Finish Workout</button>
    </div>
  );
}

function TopMetric({ icon: Icon, value, label }: { icon: typeof Dumbbell; value: string; label: string }) {
  return <div className="flex items-center justify-center gap-3 px-2"><ExerciseIcon icon={Icon} compact /><div><div className="text-2xl font-black">{value}</div><div className="text-sm text-slate-400">{label}</div></div></div>;
}
...
function InputBox({ label, value, unit }: { label: string; value: string; unit: string }) {
  return <div className="rounded-xl border border-white/10 bg-white/[0.04] p-3"><div className="text-sm text-slate-400">{label}</div><div className="mt-2 flex items-end gap-2"><span className="font-mono text-5xl font-black">{value}</span><span className="pb-2 text-sm text-slate-400">{unit}</span></div></div>;
}
