...
- `packages/core`: shared TypeScript types, validation schemas, fitness math, analytics engine, PR detection, importers, and body analysis interface.
...
The shared analytics engine lives in `packages/core/src/analytics` so future desktop, mobile, and sync clients can reuse the same formulas. UI-only aggregation remains in `apps/desktop/src/lib/analytics.ts` and is adapted through `apps/desktop/src/features/analytics/useTrainingAnalytics.ts`.
